...
def extract_info_from_text(file_text):
    ...
    try:
        ...
        biblio = file_text[start_biblio:].strip()  # Extraire tout après le mot-clé "Références"
    except ValueError:
        # Si "Références" n'est pas trouvé
    ...
    if discussion_start is not None:
        # Chercher le début de la section "Références" pour limiter la fin de la section "Discussion"
        try:
            end_discussion = file_text.index("References", discussion_start)
            discussion = file_text[discussion_start:end_discussion].strip()
        except ValueError:
            # Si "Références" n'est pas trouvé, prendre tout jusqu'à la fin
